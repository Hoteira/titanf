/*
This is pretty much a 1:1 of fontdue's bench test in order to have an accurate comparison
*/
...
const FUNCTIONS: [SetupFunction; 4] = [setup_titanf, setup_rusttype, setup_ab_glyph, setup_fontdue];
...
fn setup_titanf(group: &mut BenchmarkGroup<WallTime>, font_label: &str, font: &[u8], size: f32) {
    use titanf::TrueTypeFont;
    let mut font = TrueTypeFont::load_font(font).unwrap();
    let parameter = format!("titanf {} {}px", font_label, size);
    group.bench_function(BenchmarkId::from_parameter(parameter), |b| {
        b.iter(|| {
            let mut len = 0;
            for character in MESSAGE.chars() {
                let (_, bitmap) = font.get_char::<false>(character, size);
                len += bitmap.len();
            }
            len
        });
        ...
        b.iter(|| {
            ...
            for character in MESSAGE.chars() {
                let glyph: Glyph = font
                    .glyph_id(character)
                    .with_scale_and_position(size, point(0.0, 0.0));
